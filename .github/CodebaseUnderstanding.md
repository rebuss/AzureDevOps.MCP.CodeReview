...
| `REBUSS.Pure\Cli\InitCommand.cs` | CLI `init` command: generates MCP config files, copies prompt/instruction files, runs auth flow; auto-detects IDEs (`DetectsVsCode`, `DetectsVisualStudio`, `DetectsClaudeCode`); supports `--ide vscode`/`--ide vs`/`--ide claude` explicit targeting; Claude Code uses `mcpServers` key via `McpConfigTarget.UseMcpServersKey`; global mode (`-g`) writes to `~/.mcp.json` (VS), `%APPDATA%\Code\User\mcp.json` (VS Code), `~/.claude/.mcp.json` (Claude Code) via `ResolveGlobalConfigTargets()`; constants: `McpConfigFileName = "mcp.json"`, `ClaudeCodeMcpConfigFileName = ".mcp.json"`, `VsGlobalMcpConfigFileName = ".mcp.json"`, `ClaudeCodeMarkerFile = "CLAUDE.md"` |
