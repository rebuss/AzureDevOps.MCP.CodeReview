...
Owns background enrichment of pull-request diffs so `get_pr_metadata` can fall back to a basic-summary response on internal timeout while enrichment continues, and `begin_pr_review` can later reuse the in-flight or completed work with its own fresh timeout window. Mirrors `RepositoryDownloadOrchestrator` precedent: singleton with mutable per-PR state, background body runs under `IHostApplicationLifetime.ApplicationStopping`, never the caller's `CancellationToken`. Documented Principle VI deviation in plan.md.
...
| `REBUSS.Pure\Services\PrEnrichment\PrEnrichmentResult.cs` | Immutable record: `SortedCandidates`, `EnrichedByPath`, `Allocation`, `SafeBudgetTokens`, `CompletedAt`. `HeadSha` field is sourced from `FullPullRequestMetadata.LastMergeSourceCommitId` | `PackingCandidate`, `PageAllocation` |
...
| `REBUSS.Pure\Cli\Prompts\review-pr.prompt.md` | Embedded resource: PR review prompt template; rewritten in feature 015 for the stateful review session lifecycle (`begin_pr_review` → loop(`next_review_item` + `record_review_observation`) → `submit_pr_review`) with explicit "Hard rules" section for small-context agents |
...
| `REBUSS.Pure.SmokeTests\Protocol\ToolsListProtocolTests.cs` | Protocol tests (no credentials): tools/list — tool count (10), names, schemas; PrNumberTools checks property declaration for get_pr_metadata; PaginationEnabledTools array; ContentTools_HavePageNumberProperty for get_local_content |
...
// GetPullRequestMetadataToolHandler, GetLocalContentToolHandler,
// BeginPullRequestReviewToolHandler, NextReviewItemToolHandler,
// RecordReviewObservationToolHandler, SubmitPullRequestReviewToolHandler,
// RefetchReviewItemToolHandler, QueryReviewNotesToolHandler
