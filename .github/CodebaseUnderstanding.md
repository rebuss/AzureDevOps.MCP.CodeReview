...
| `REBUSS.Pure\Services\AgentInvocation\CopilotAgentInvoker.cs` | `IAgentInvoker` implementation wrapping `ICopilotSessionFactory`. Creates a fresh session per call, subscribes to `AssistantMessageEvent` / `SessionIdleEvent` / `SessionErrorEvent`, sends the prompt, returns the accumulated response text, disposes the session. Extracted from the previously inlined session+events block in `CopilotPageReviewer` / `FindingValidator` so both classes can be refactored to use the common seam (TODO — wiring not yet complete, see `Program.cs` note). Requires a non-empty `model` parameter. |
...
// TODO: thread through CopilotPageReviewer + FindingValidator so --agent claude
// actually drives runtime review. Today the wiring only affects init-time config.
if (agent == "claude")
