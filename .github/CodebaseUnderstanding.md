...
| `REBUSS.Pure\Cli\CopilotCliSetupStep.cs` | Feature 012 — optional GitHub Copilot CLI setup step run at the end of `InitCommand.ExecuteAsync` regardless of SCM provider or `--pat`. Detects `gh` / auth / `gh copilot` extension state fresh on every run (no persisted decline), prompts the user, and installs the `github/gh-copilot` extension on consent. All internal `gh auth login --web` calls pass `-s copilot` so fresh logins grant the Copilot OAuth scope. When the extension install fails because newer `gh` CLI ships copilot as a built-in command (error: "matches the name of a built-in command"), falls back to running `gh copilot --version` interactively to trigger the built-in installation prompt. `VerifyCopilotSessionAsync` runs a `CopilotVerificationRunner` probe against the SDK; on a `NotAuthenticated + LoggedInUser` verdict it invokes `gh auth refresh -h github.com -s copilot` interactively once and re-probes before deciding whether to print the failure banner — this recovers sessions whose `gh auth login` happened outside `init` and therefore lacked the Copilot scope. Decline, failure, or non-interactive stdin are soft exits with an informational banner; the step never affects init's exit code. Reuses `GitHubCliProcessHelper.TryFindGhCliOnWindows` for fresh-install PATH recovery. |
